// digimon_optimal_vaccine_path.js
// Script to find the optimal Vaccine Digimon evolution path for best move coverage against Virus Digimon in Cyber Sleuth
// Usage: node digimon_optimal_vaccine_path.js
...
// Get all Virus Digimon and their attributes
const virusDigimon = digimonList.filter(d => d.Type === 'Virus');
const virusAttributes = virusDigimon.map(d => ({ Name: d.Name, Attribute: d.Attribute }));

// Get all Vaccine Digimon
const vaccineDigimon = digimonList.filter(d => d.Type === 'Vaccine');

// For each Vaccine Digimon, collect all possible inherited moves
...
// For Patamon path: Patamon -> Angemon -> MagnaAngemon -> Seraphimon
const evoPath = ['Patamon', 'Angemon', 'MagnaAngemon', 'Seraphimon'];
...
for (const move of inheritedMoves) {
  ...
  if (!attr) continue;
  // Count how many Virus Digimon are weak to this move
  const count = virusAttributes.filter(v => effectiveness[attr] && effectiveness[attr].includes(v.Attribute)).length;
  coverage[move] = { attribute: attr, virus_covered: count };
}

// Sort moves by coverage
const sortedMoves = Object.entries(coverage).sort((a, b) => b[1].virus_covered - a[1].virus_covered);

console.log('Optimal Patamon Evolution Path (Vaccine): Patamon -> Angemon -> MagnaAngemon -> Seraphimon');
console.log('Inherited Moves and Coverage Against Virus Digimon:');
for (const [move, info] of sortedMoves) {
  console.log(`- ${move} [${info.attribute}] covers ${info.virus_covered} Virus Digimon`);
}

console.log('\nSummary: Use move inheritance to teach Patamon moves of multiple elements, then evolve to Seraphimon for best coverage against Virus Digimon. Prioritize Light moves for Dark Virus, Wind for Earth Virus, and others as needed.');
