...
window.initEvolutionpathTab = function () {
  ...
  Promise.all([
    fetch(window.DIGIMON_DATA_PATH_DATA).then(r => {
      if (!r.ok) throw new Error('Failed to fetch digimon_data.csv');
      return r.text();
    }),
    fetch(window.DIGIMON_ICON_MAP_PATH_DATA).then(r => {
      if (!r.ok) throw new Error('Failed to fetch digimon_icon_map.csv');
      return r.text();
    }),
    fetch(window.DIGIMON_MOVES_COMPLETE_PATH_DATA).then(r => {
    ...
    calcBtn.onclick = function () {
      ...
      function runPathfindingWithMoves(movesToUse) {
          ...
          Promise.all([
            fetch(window.DIGIMON_EVOLUTIONS_PATH_DATA).then(r => r.text()),
            fetch(window.DIGIMON_MOVES_PATH_DATA).then(r => r.text())
...
window.getMovesFromThreatAnalysis = function(initialDigimon, finalDigimon, topN = 5) {
  ...
  return Promise.all([
  fetch(window.DIGIMON_MOVES_PATH_DATA).then(r => r.text()),
  fetch(window.DIGIMON_MOVES_COMPLETE_PATH_DATA).then(r => r.text())
